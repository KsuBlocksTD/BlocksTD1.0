...
public class ConfigOptions {
    ...
    * If the maximum desired tier of an effect is n, use n-1 as the default
    ...
    public int getSpeedMaxLevel() {
        return config.getInt("btd.attributes.upgrades.player.speed.max-level", 5);  // Default value is 5 if not set
    }
    public int getSpeedBaseCost() {
        return config.getInt("btd.attributes.upgrades.player.speed.cost", 500); // Default value is 500 if not set
    }


    // Getter for Slowness level max upgrade
    public int getSlownessMaxLevel() {
        return config.getInt("btd.attributes.upgrades.player.sword.slowness.max-level", 5);  // Default value is 5 if not set
    }
    ...
    public boolean getSlownessDurationIncreaseOnUpgrade() {
        return config.getBoolean("btd.attributes.upgrades.player.sword.slowness.duration.increase-duration-on-upgrade", false); // Default value is false if not set
    }


    // Getter for Strength level max upgrade
    public int getStrengthMaxLevel() {
        return config.getInt("btd.attributes.upgrades.player.strength.max-level", 5);  // Default value is 5 if not set
    }
    public int getStrengthBaseCost() {
        return config.getInt("btd.attributes.upgrades.player.strength.cost", 750);
    }


    // Getter for Sweeping-Edge level max upgrade
    public int getSweepingEdgeMaxLevel() {
        return config.getInt("btd.attributes.upgrades.player.sword.sweeping-edge.max-level", 5);  // Default value is 5 if not set
    }
    public int getSweepingEdgeBaseCost() {
        return config.getInt("btd.attributes.upgrades.player.sword.sweeping-edge.cost", 400);  // Default value is 600 if not set
    }


    public int getSwordMaterialMaxLevel() {
        // 1: wood, 2: stone, 3: iron, 4: diamond, 5: netherite
        return config.getInt("btd.attributes.upgrades.player.sword.material.max-level", 5);  // Default value is 5 if not set
    }
    public int getSwordMaterialBaseCost() {
        // 1: wood, 2: stone, 3: iron, 4: diamond, 5: netherite
        return config.getInt("btd.attributes.upgrades.player.sword.material.cost", 400);  // Default value is 400 if not set
    }
    ...
    public int getBasicTowerCost(){
        return config.getInt("btd.attributes.upgrades.towers.basic.cost", 500);
    }

    public int getFastTowerCost(){
        return config.getInt("btd.attributes.upgrades.towers.fast.cost", 300);
    }

    public int getSniperTowerCost(){
        return config.getInt("btd.attributes.upgrades.towers.sniper.cost", 750);
    }

    public int getSplashTowerCost(){
        return config.getInt("btd.attributes.upgrades.towers.splash.cost", 900);
    }

    public int getSlowTowerCost(){
        return config.getInt("btd.attributes.upgrades.towers.slow.cost", 450);
    }
